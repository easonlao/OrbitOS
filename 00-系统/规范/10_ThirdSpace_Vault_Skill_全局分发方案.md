...
```
vault/00-系统/Skills/orbit-vault/   ← canonical 源（唯一维护点）
         ↓  symlink                ↓  symlink          ↓  symlink
~/.claude/skills/          ~/.cursor/rules/      ~/.opencode/skills/
orbit-vault           orbit-vault      orbit-vault
...
```bash
VAULT="/path/to/vault"
SKILL="$VAULT/00-系统/Skills/orbit-vault"

# 按需选择对应 Agent 平台
mkdir -p ~/.claude/skills
ln -sf "$SKILL" ~/.claude/skills/orbit-vault        # Claude Code

mkdir -p ~/.cursor/rules
ln -sf "$SKILL" ~/.cursor/rules/orbit-vault         # Cursor

mkdir -p ~/.opencode/skills
ln -sf "$SKILL" ~/.opencode/skills/orbit-vault      # OpenCode
...
| 占位符 | 含义 | 解析方式 |
|--------|------|---------|
| `{VAULT}` | vault 根目录 | 向上遍历找 `.orbit/workspace-index.yaml` |
...
- **更新 Skill**：只改 `vault/00-系统/Skills/orbit-vault/`，软链自动生效
- **新增 Agent 支持**：`ln -sf "$SKILL" ~/.<agent>/skills/orbit-vault`
- **触发词调整**：修改 `orbit-vault/SKILL.md` 的 `triggers:` 字段
- **Scripts 更新**：直接改 `scripts/orbit-vault.mjs`，全平台即时生效
