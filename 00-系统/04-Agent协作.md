...
Agent 之间需要继续交接的事项，统一写到 [[agents/BOARD|Agent 交接板]]。具体 handoff 实例放在 `00-系统/agents/handoff/`，模板源放在 `.orbitos/templates/00-系统/agents/handoff/TEMPLATE.md`。
...
当工作需要跨 Agent 继续时，先输出一份 handoff，再由审核把关后，由下一位 Agent 阅读最后确认后接手。
