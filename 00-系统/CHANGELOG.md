...
- 更新项目目录约定：当前状态、路线和任务直接放在项目根层，专项记录放在 `docs/`，实际 release/product Git 仓库放在 `repo/`。
- 新增根目录临时文件边界：agent 不得在 OrbitOS 根目录创建临时目录、外部仓库 clone、下载物或实验输出；需要留存的临时材料进 `01-收件箱/`，可丢弃 scratch 用系统临时目录或 `.orbitos/tmp/`。
