...
check "workspace-index.yaml 存在"         test -f "$VAULT/.orbit/workspace-index.yaml"
check "schema/ 包含 5 个文件"             bash -c "[ $(ls $VAULT/.orbit/schema/*.yaml 2>/dev/null | wc -l) -eq 5 ]"
check "Skills/orbit-vault 存在"      test -d "$VAULT/00-系统/Skills/orbit-vault"
...
check "schema/ 无硬编码路径" \
  bash -c "! grep -r '/Users/\|/home/' $VAULT/.orbit/ 2>/dev/null | grep -qv '#'"
...
check "orbit-vault.mjs 无硬编码路径" \
  bash -c "! grep -q '/Users/' $VAULT/00-系统/Skills/orbit-vault/scripts/orbit-vault.mjs"

echo ""

# ── STEP 3: Node.js 脚本 ─────────────────────────────────────
cyan "STEP 3: Node.js 脚本测试"
SCRIPT="$VAULT/00-系统/Skills/orbit-vault/scripts/orbit-vault.mjs"
...
check "resolve-vault（无 vault 时报错，不 fallback）" \
  bash -c "cd /tmp && ORBIT_VAULT= node $SCRIPT resolve-vault --cwd . 2>&1 | grep -qiE 'error|Cannot resolve'"
...
ORBIT_VAULT="$VAULT" git commit -m "test: verify hook creates worklog" -q
