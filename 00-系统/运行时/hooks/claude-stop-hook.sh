...
# ── 检测 vault（向上遍历查找 .orbit/workspace-index.yaml）──
find_vault() {
    local dir="${1:-$PWD}"
    while [ "$dir" != "/" ]; do
        if [ -f "$dir/.orbit/workspace-index.yaml" ]; then
            echo "$dir"
            return 0
        fi
        dir=$(dirname "$dir")
    done
    # 兜底：$ORBIT_VAULT 环境变量（兼容旧名 $orbit_VAULT）
    local env_vault="${ORBIT_VAULT:-${orbit_VAULT:-}}"
    if [ -n "$env_vault" ] && [ -f "$env_vault/.orbit/workspace-index.yaml" ]; then
        echo "$env_vault"
        return 0
    fi
    # 兜底：~/.orbit/config.yaml
    local configured=""
    configured=$(awk -F: '/^(default_vault|vault_root|vault):/{gsub(/^[ \t"]+|[ \t"]+$/, "", $2); print $2; exit}' "$HOME/.orbit/config.yaml" 2>/dev/null || true)
    if [ -n "$configured" ] && [ -f "$configured/.orbit/workspace-index.yaml" ]; then
        echo "$configured"
        return 0
    fi
    return 1
...
{
  "decision": "block",
  "reason": "检测到本次 session 有重大产出。请将关键内容追加到今日工作日志（02-日记/工作日志/YYYYMMDD_工作日志_周X.md）的「重点记录」章节，使用规范格式：### HH:MM — 标题；随后写明 记录方、写入方式、会话ID、来源目录，以及 **为什么做** / **怎么做的** / **改了什么**。若会话ID取不到，写 unknown。用 Edit 工具直接追加写入。写完后正常停止。"
}
EOF
    exit 0
fi
...
{
  "decision": "block",
  "reason": "请在停止前将本次对话关键内容写入工作日志（02-日记/工作日志/YYYYMMDD_工作日志_周X.md）的「重点记录」章节，并补齐 记录方、写入方式、会话ID、来源目录。若会话ID取不到，写 unknown。写完后正常停止。"
}
EOF
    exit 0
fi
