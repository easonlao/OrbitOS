...
1. **File path**: A path to an inbox note (e.g., `/kickoff 01-收件箱/MyIdea.md`) - read the file contents
2. **Inline text**: A short description of a project idea (e.g., `/kickoff Build a habit tracker app`)
3. **No input**: If nothing provided, list files from `01-收件箱/` and ask the user to select one
...
Follow these steps:
1. Gather Context: Search 04-项目 and 02-日记 for existing notes related to this idea
2. Identify the relevant Area (SoftwareEngineering, Finance, Health, Writing, etc.)
3. Create the plan file at 04-项目/Plan_YYYY-MM-DD_Kickoff_<ProjectName>.md using this format:
...
prompt: "Execute the project kickoff plan located at: 04-项目/Plan_YYYY-MM-DD_Kickoff_<ProjectName>.md

Instructions:
1. Read the plan file
2. Note any user modifications or answered clarification questions
3. Create the project note:
   - For small projects: Create 04-项目/<ProjectName>.md
   - For medium/large projects: Create 04-项目/<ProjectName>/<ProjectName>.md
...
5. Link the project in today's daily note at 02-日记/工作日志/YYYYMMDD_工作日志_周X.md
6. Archive the plan: keep in 04-项目/ with status: archived, or move to 99-归档/
7. If this kickoff originated from an inbox item (01-收件箱/):
   - Update the inbox file's frontmatter: set status: processed
   - Move the file to 99-归档/收件箱/YYYY/MM/ (use the current date for year/month)
