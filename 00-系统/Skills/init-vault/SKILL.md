...
vault 根必须存在以下文件，缺失则从 `.orbit/schema/` 下的模板补全：

```
.orbit/workspace-index.yaml     ← vault 根锚点
...
**作用**：让 Agent 从 vault 目录**之外**也能加载 orbit-vault Skill。

**需要做的事**：将 `./00-系统/Skills/orbit-vault` 链接或复制到 Agent 平台的全局 skills 目录。

不同 Agent 平台位置不同，由 Agent 自行判断。

### 6. Obsidian 插件启用（需用户手动）

插件已随 vault 分发，位于：`.obsidian/plugins/orbit-dashboard/`

用户需要在 Obsidian 中手动启用（无法自动化）：
设置 → 第三方插件 → 允许社区插件 → 启用 OrbitOS Dashboard
...
# 2. Schema 完整性
find .orbit/schema -type f | sort

# 3. 无硬编码绝对路径（期望：0）
grep -r "/Users/\|/home/\|C:\\\\" .orbit/ CLAUDE.md AGENTS.md 2>/dev/null | grep -v ".git" | wc -l
```
