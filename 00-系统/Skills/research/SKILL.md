...
## 输出结构
- 主笔记: 03-知识/<领域>/<主题>/<主题>.md
- 原子概念: 03-知识/<分类>/<概念名称>.md
- 示例/资源: 03-知识/<领域>/<主题>/examples/（如需要）
- 原始材料: 03-知识/<领域>/<主题>/原始材料/（存放搜索抓取的网页原文或关键摘要）
...
prompt: "Execute the research plan located at: 04-项目/Plan_YYYY-MM-DD_Research_<Topic>.md
...
3. Create the Main Research Note:
   - Path: 03-知识/<Area>/<Topic>/<Topic>.md
   - Sections to include:
...
4. Create Atomic Wiki Notes:
   - For each reusable concept: 03-知识/<Category>/<ConceptName>.md
   - Keep concise (1-3 paragraphs)
   - Include '相关概念' section with related links

5. Save Raw Materials (optional):
   - Only for pages with irreplaceable data, formulas, or at risk of deletion
   - 03-知识/<Area>/<Topic>/原始材料/
   - Save as YYYY-MM-DD_来源_标题.md with URL and key excerpts
   - Most research: source link in wiki entry frontmatter is sufficient

6. Create Visual Map (if complex topic):
   - <Topic>_Map.canvas to visualize concept relationships

7. Create Examples (if applicable):
   - Save code examples in 03-知识/<Area>/<Topic>/examples/

8. Link & Track:
   - Append to today's Daily Note: 02-日记/工作日志/YYYYMMDD_工作日志_周X.md
   - If related to a project, add link in project's Progress section

8. Archive: Move plan to 04-项目/归档/
