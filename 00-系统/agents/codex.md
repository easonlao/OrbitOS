...
- 2026-06-15｜人读 Dashboard 点名现有可见 Markdown 时如果不用 Obsidian 双链，用户需要手动找文件。｜来源：用户纠正 Hermes PR 提醒记录｜影响：写入 `今日.md`、`下一步.md`、`待确认.md` 等人读视图时，现有可见 Markdown 文件必须给可点击双链；内部 `.orbitos/` 路径继续用代码路径。
- 2026-06-16｜`03-项目/` 下的具体项目可以按项目粒度建立独立 Git 仓库；这不等同于修改 OrbitOS 根仓库的 Git 边界。｜来源：用户纠正 OpenClash 规则重写项目｜影响：用户要求“开仓库”且目标是具体项目时，可在项目目录初始化独立 Git，并避免把项目文件混入 OrbitOS 根仓库。
