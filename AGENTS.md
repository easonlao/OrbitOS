...
6. 如果任务涉及当前用户的协作方式、输出风格或默认配合习惯，且 `00-系统/08-本地协作偏好.md` 存在，再读取该文件；它只代表当前 Runtime 的本地偏好，不自动视为系统通用规则。若文件仍带有 `TODO_UPDATE_LOCAL_COLLAB_PREFS`，先提醒用户更新模板，再决定是否参考其中内容。
7. 完成实质性工作后进入 `.orbitos/workflows/progress-sync.md`。
...
- 单个 agent 经验整理与踩坑收缩：`.orbitos/workflows/agent-self-check.md`
- 跨 agent 的公共经验巡检：`.orbitos/workflows/learned-review.md`
- 出现错误、返工、用户纠正或可复用经验时的分类逻辑：`.orbitos/workflows/experience-capture.md`
...
以上 rule 均位于 `.orbitos/rules/core/`，只在任务涉及时读取。创建、修改或审查 MAP、README、AGENTS、STATUS、ROADMAP 或 ADR 时，先读 `document-semantics.md`。
