...
如果你不确定该读什么，先读 `00-系统/MAP.md`，再按本文件路由选择，不要全量扫描 vault。
...
0. 确认 `.orbitos/agents/registry.yaml` 存在；如果不存在，停止并报告路径缺失，不读取其他 agent 的 profile。
1. 读取 `00-系统/MAP.md`、`00-系统/CONTEXT.md`、`00-系统/PRINCIPLES.md`。
2. 读取 `.orbitos/agents/registry.yaml`，只读确认自己的 `agent_id` 和部署上下文。
3. 读取自己的 `profile_ref`，查看经验记录、踩坑、待确认来源和 Learned Rule 使用记录。
...
可见 Markdown 默认需要 frontmatter；例外：`01-收件箱/粘贴.md` 是自由输入入口，不要求 frontmatter。
