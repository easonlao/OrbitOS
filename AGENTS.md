...
9. 用户显式调用 `$handoff`，或说“交给另一位 Agent 继续”“交接当前任务”“把任务交给 {agent_id}”时，判定为 `delegated`；协作模块为 `ready` 时先进入 `.orbitos/modules/collaboration/workflows/handoff-adapter.md`，再由它调用正式交出流程。未指定回交对象时，完成结果默认交回原交出 Agent 验收。用户说“获取交接工作”“接手交接”或“查看待接手任务”时，协作模块为 `ready` 则进入 `.orbitos/modules/collaboration/workflows/handoff-pickup.md`。两种情况均不得要求用户指定目录或 handoff 文件；模块未就绪时停止并说明缺口。
