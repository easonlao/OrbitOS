...
- E2 思考方法层：`03-项目/OrbitOS/docs/E2-THINKING-MODES-DRAFT.md`；默认采用自动推荐、显式可见、用户可覆盖的调用方式；它不是额外 workflow

### 维护 Rule

- 处理 `03-项目/{project}/`：`project-management.md`
- Hindsight highlight 与直连 API：`hindsight-highlight.md`
...
- Hindsight 默认直接走 HTTP API；highlight 触发门、retain / skip 边界与 API 格式见 `.orbitos/rules/core/hindsight-highlight.md`。
