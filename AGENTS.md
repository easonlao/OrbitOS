...
8. 如果任务命中思考触发器，先读取 `.orbitos/rules/core/thinking.md` 并展示思考模式启动选择；用户选定方法或明确说“直接做”前，不进入实质分析、设计、写入或决策。
...
- 思考方法层：`.orbitos/rules/core/thinking.md`；默认自动推荐、显式可见、用户可覆盖；它不是额外 workflow 或可选模块
