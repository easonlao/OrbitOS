...
- E2 思考方法层：`03-项目/OrbitOS/docs/DESIGN.md` 中的 E2 思考方法层；默认采用自动推荐、显式可见、用户可覆盖的调用方式；它不是额外 workflow
...
以上 rule 均位于 `.orbitos/rules/core/`，只在任务涉及时读取。创建、修改或审查 MAP、README、AGENTS、STATUS 或 ADR 时，先读 `document-semantics.md`。
