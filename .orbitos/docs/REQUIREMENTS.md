...
- 完成凭证应由脚本根据少量任务结果生成，agent 不应手写完整 event 结构。
...
- agent 必须根据任务语义定位工作区域，并沿根目录到目标路径读取局部 `AGENTS.md`；不得通过全量扫描发现入口。
- 只有任务涉及时才加载对应 rule 或 workflow。
- 关键字段缺失、需要移动用户内容、改变 Git 边界或创建正式产物时必须停止并请求确认。
- 写入型 workflow 必须通过 validation；失败时不得刷新人读视图。
- 系统只在正式知识、用户内容移动、规则提升、Git 边界等高风险写入点要求确认，不为普通工作增加全程状态机。
- 用户只需用自然语言提出任务，不需要主动触发项目工作流或同步命令。
- agent 自动判断任务是当场完成、需要跨会话继续，还是形成长期产品目标；需要跨会话的当前事项进入 STATUS，当场完成的小修改只留下 event。
- 新增长期目标、改变 ROADMAP 完成条件、替换当前事项或决定现在推进哪个路线目标，必须由用户确认；agent 只能提出建议和维护流转。
...
- 用户打开今日并进入对应区域的权威状态入口后，能理解当前状态、待确认事项和下一步。
