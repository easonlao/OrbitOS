...
运行态私有 agent 工作目录优先使用本地 `.git/info/exclude` 保持隔离，而不是默认提升为 Product Repo 的共享 ignore 规则。
...
- Product Repo 内的 ADR：重大决策的背景、备选方案与取舍。

原件是内容证据，event 是操作证据，ADR 是决策依据。三者不能互相替代。
...
- `.orbitos/rules/core/`：所有区域共同遵守的稳定约束。
- `.orbitos/rules/areas/`：收件箱、时间线、项目、知识、资源等区域各自的数据契约。
...
时间线是聚合视图，不是操作事实账本或任务源；项目 STATUS 是项目当前状态源，不是历史日志。

## 4. 区域契约与数据流

每个可见区域有不同的数据职责，因此使用独立区域规则；跨区域共同约束仍由 core rule 统一定义。

- 区域规则定义该区域接收什么、以什么为权威来源、允许流向哪里、可以生成哪些投影以及禁止什么。
- workflow 执行一次具体的区域内或跨区域流转，并处理确认、失败和验证。
- 根 `AGENTS.md` 根据任务对象加载最小区域规则；跨区域任务同时加载源区域、目标区域和匹配 workflow。
- 可见目录不因区域规则而默认增加 `AGENTS.md`；只有目录树存在必须逐级继承的局部约束时才创建。
...
投影不能反向覆盖权威来源。待确认事项保留在产生它的区域状态中；当天确实需要用户处理时，`今日.md` 只提供摘要和入口。
...
```text
已确认需求或决策 -> ROADMAP 定义里程碑目标与出口
实际工作 -> event 记录操作事实
出口验证 + event -> STATUS 更新详细状态与判断依据
STATUS -> ROADMAP 更新总体状态
STATUS 当日变化 -> 今日投影摘要
已发布产品变化 -> CHANGELOG 记录版本历史
```

- ROADMAP 保存目标、完成条件和总体状态，供用户快速查看；它不保存任务细节或完成证据。
- event 是操作证据，但单个 event 不自动等于里程碑完成；必须同时满足 ROADMAP 出口。
- STATUS 是 event 与完成条件形成的详细项目镜像；当前任务通过路线编号映射到 ROADMAP。
- ROADMAP 的总体状态由 STATUS 投影；两者在同一次 Progress Sync 中更新，不能各自独立判断。
- 今日只读取 STATUS 的当日变化，不反向覆盖项目状态。
- CHANGELOG 只记录已经进入发布版本的产品变化，不接收未发布任务或项目计划。
