...
- 用测试 bank 验证规范，不污染现有主 bank 或个人历史 bank。
...
- OrbitOS 后期建议只使用一个正式主 bank。
- 正式主 bank 应由用户明确命名和确认。
- 测试 bank 只用于验证规则和接入路径，不代表长期多 bank 架构。
- 已存在的个人 / 历史 bank 不应默认作为 OrbitOS 新规范的事实基线。
...
- `{legacy_bank}` 如存在，首阶段只读 recall 为主。
- `{test_bank}` 只用于 Hindsight Bridge 试点，默认可使用 `orbitos-test`。
- 不从 `{legacy_bank}` 批量导入旧数据。
- 不在 `{test_bank}` 开启 auto retain。
...
- 显式 bank 路径可以保证试点写入 `{test_bank}`，不污染 `{primary_bank}` 或 `{legacy_bank}`。
- HTTP API 不依赖单个 agent 的插件实现，适合多个 agent 共享同一套语义规则。
- 如果某个 agent 的默认 retain 工具绑定到固定 bank，不能把它作为测试 bank 写入路径，除非工具能显式指定 bank。

首版禁止：

- 不用无法显式指定 bank 的默认 retain 工具写试点记忆。
...
```text
{hindsight_base_url}/mcp/{bank_id}/
```

示例：

```text
{hindsight_base_url}/mcp/{test_bank}/
{hindsight_base_url}/mcp/{primary_bank}/
```
...
- 测试期 `{test_bank}` 和 `{primary_bank}` 可通过不同 MCP server 配置隔离。
...
- 目标 endpoint 必须是 `{test_bank}` single-bank endpoint，除非用户明确确认写其他 bank。
...
1. 连接 `{hindsight_base_url}/mcp/{test_bank}/`。
2. `tools/list` 中确认存在 recall / retain / list tags / operations 类工具。
3. 用 MCP recall 查询一条已确认写入 `{test_bank}` 的测试记忆，确认命中。
4. 不执行 MCP retain，除非用户明确确认。
5. 确认 MCP 不会访问或写入 `{primary_bank}` 或 `{legacy_bank}`。
...
试点阶段固定为 `{test_bank}`，默认可使用 `orbitos-test`。

如果工具或插件无法指定 bank，必须停止并报告，不得退回写入 `{primary_bank}` 或 `{legacy_bank}`。
...
orbitos/runtime/{agent_id}-runtime-path
...
- 不在试点阶段写入 `{primary_bank}` 或 `{legacy_bank}`，除非用户明确确认。
- 不启用 auto retain。
