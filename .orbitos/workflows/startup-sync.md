...
7. 读取 `02-时间线/今日.md`。只有今日摘要不足，或任务明确涉及确认事项与后续入口时，才展开 `待确认.md` 或 `下一步.md`。
8. 任务明确涉及具体项目时，再读取该项目的 `README.md`、`STATUS.md` 和任务直接相关文件。
9. 输出短摘要：`agent_id`、runtime 状态、当前状态、待确认、下一步。
