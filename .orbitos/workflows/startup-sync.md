...
- 读取当前 agent 的轻量 profile，并知道完整经验入口在何处。
- 输出短状态摘要。
...
1. 如果 `agent_id` 存在：
   - 读取对应 `deployment`。
   - 读取对应 `profile_ref`。
   - 只读取 `profile_ref` 中的部署信息、当前定位、启动关注和经验入口。
   - 不默认展开完整经验文件；只有任务命中相关领域、出现失败返工或需要排查历史问题时，才按经验入口按需读取。
   - 运行或读取 runtime 环境检查：
...
- [ ] 已准备读取当前 agent profile 的部署信息、启动关注和经验入口。
...
- [ ] 已读取当前 agent 的 `profile_ref`，并确认部署信息、启动关注和经验入口。
- [ ] 如无失败、返工或明确领域命中，未默认展开完整经验文件。
...
- 无法读取 registry 时读取其他 agent 的 profile 或经验文件。
