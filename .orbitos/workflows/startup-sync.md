...
8. 如果 `00-系统/09-人物档案.md` 存在且仍处于 `baseline_status: pending`，提醒用户：动态人物模块尚未完成首轮问卷，当前只是一份待初始化骨架。
9. 如果 `00-系统/08-本地协作偏好.md` 存在且仍包含 `TODO_UPDATE_LOCAL_COLLAB_PREFS`，提醒用户这是首次初始化生成的模板，请先按自己的习惯更新后再依赖。
10. 输出短摘要：`agent_id`、runtime 状态、当前状态、待确认和可继续入口。
