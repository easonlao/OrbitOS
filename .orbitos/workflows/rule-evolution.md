...
- 让 agent 自身经验先在 agent experience 文件中积累。
...
轻量 profile `00-系统/agents/{agent_id}.md` 只负责启动定位、部署信息和经验入口，不直接承载完整经验日志。
...
5. 有来源：来自 agent experience、event、validation failure、用户确认或多 agent 重复经验。

不满足时：

- 太具体：留在对应 agent experience。
...
详细证据和长说明保留在 agent experience、event 或相关文档中，INDEX 只做索引和判断。

## 触发条件

以下情况触发 Rule Evolution 检查：

- Agent Self Check 判断某条候选可能进入 learned。
- 用户说“提炼规则”“同步规则”“这个以后要记住”“规则演化”。
- agent experience 中新增了可复用经验或待确认来源。
- 多个 agents 在相似位置重复踩坑。
...
Progress Sync 不负责触发 Rule Evolution。
...
如果候选还不成熟，结果保持为 `candidate_only`，留在 agent experience 或今日待确认，不写 learned index。

## 闭环总览

```text
agent 工作 / 失败 / 成功经验
  -> 记录到 agent experience
  -> agent-self-check / learned-review 判断是否进入规则层
...
- 不在 experience 中直接宣布系统通用规则。
- 如果需要用户确认，标明“待确认”，并由 learned review 或其他对应流程投影到 `今日.md`。
...
- 总表只写判断和索引，不写长解释。
- 详细证据保留在 agent experience、event 或相关文档。
...
learned review 或专项规则整理时，从以下位置收集候选：
...
每次 Rule Evolution 产生以下结果时，对应执行流程必须刷新 `今日.md`：

- 新 learned rule 已写入。
- learned rule 有失败或冲突。
- learned rule 建议提升 core。
- 用户需要判断是否把 agent experience 中的经验提炼成系统规则。
...
- [ ] 已确认有 Experience Capture、agent experience、event、validation failure 或用户触发来源。
- [ ] 已确认候选不是一次性偏好或普通流水账。
- [ ] 已读取相关 agent experience 和必要证据。

### 执行检查

- [ ] 已完成初筛：profile / learned / workflow / skill / docs / 今日待确认。
- [ ] 如进入 learned，已保持规则原子化并更新 `.orbitos/rules/learned/INDEX.md`。
- [ ] 如只是 agent 私有经验，已保留在 agent experience。
- [ ] 如需要用户判断，已投影到 `今日.md`。
- [ ] 已在对应 event 或留痕中记录本次规则整理结果。
...
- 不让待确认事项只停留在 agent experience 而不投影到 `今日.md`。
