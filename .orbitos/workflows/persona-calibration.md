...
3. 运行 `python .orbitos/scripts/persona/calibrate.py --source 00-系统/09-人物档案.md --runtime .`。
4. 读取新增的建议，向用户说明：
   - 观测到什么行为证据
   - 与哪条假设或基线有冲突
   - 建议如何理解或后续如何确认
5. 如果用户暂不处理，建议保留 `open` 状态，等待后续更多证据。
6. 如果用户已明确判断，应转入 `persona-update.md`，而不是在本流程里直接改 confirmed 或 baseline。
