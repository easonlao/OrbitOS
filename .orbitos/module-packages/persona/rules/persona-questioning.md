...
- 对用户的第一步提问优先使用选择题，不先抛抽象开放题。
- 在用户没有明确要求批量回答时，默认一次只问 1 题。

## 标准流程

1. 先说明边界：这是一份 MBTI 种子问卷，不是官方 MBTI，也不是最终人格定论。
2. 先让用户从预设 `identity` 模板里选 1 个最接近的选项；如果都不完全贴切，再允许补 1 句修正。
3. 进入 24 题问卷后，按 `python .orbitos/modules/persona/scripts/mbti.py --print-json` 导出的标准题面逐题提问。
4. 每题只接受 `1 / 2 / 3 / 4 / 5` 五种回答，其中 `3` 明确表示不确定 / 看情况。
...
- `.orbitos/modules/persona/workflows/persona-calibration.md` 与 `.orbitos/modules/persona/workflows/persona-update.md` 不使用这套逐题问法。
