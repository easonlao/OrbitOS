...
在用户明确启用动态人物模块时，用一轮 24 题、5 档垂直选项的基线问卷生成当前工作副本专属的 `00-系统/09-人物档案.md`，并把协作相关的派生结论投影到 `00-系统/08-本地协作偏好.md`。
...
3. 先给用户一组稳定底色模板，让用户选 1 个最接近的选项；如有必要，再补 1 句修正，整理成 `--identity` 的输入。
4. 先读取 `.orbitos/modules/persona/rules/persona-questioning.md`，再按 `python .orbitos/modules/persona/scripts/mbti.py --print-json` 导出的标准题面逐题收集答案。
5. 默认一次只问 1 题；除非用户明确要求批量回答，否则不得一次混问多题。
6. 允许的答案档位只有 5 种：`1 / 2 / 3 / 4 / 5`。
   - `1`：明显更接近左侧 / A
   - `2`：略微更接近左侧 / A
   - `3`：不确定 / 看情况
   - `4`：略微更接近右侧 / B
...
9. 运行 `python .orbitos/modules/persona/scripts/baseline.py --source 00-系统/09-人物档案.md --identity ... --answers ...` 生成主源。
10. 如当前工作副本已有 `00-系统/08-本地协作偏好.md`，再运行 `python .orbitos/modules/persona/scripts/project.py --source 00-系统/09-人物档案.md --runtime . --collab` 写入派生协作段落。
...
- [ ] 已向用户说明 MBTI 仅作种子假设。
- [ ] 已读取 `.orbitos/modules/persona/rules/persona-questioning.md`。

### 执行检查

- [ ] 已通过选择式引导收集初始底色；如有必要，已补 1 句修正。
- [ ] 已按 24 题、5 档垂直选项格式收集答案。
