...
name: book-ingest
description: >-
  把用户提供的书源文件(epub / mobi / azw / azw3 / prc / md / markdown / 文字型 pdf)或已拆章节集合导入,
...
- `md / markdown / txt`
- 文字型 `pdf`
- 已拆章节集合（保持已有章节顺序，不重新拆分）

## OrbitOS 集合输入

`book-ingest` 仍是唯一导入入口。若输入已经是一组按章节拆好的 Markdown 原文，不把它们重新合并或交给切分脚本；使用 `scripts/import_presplit_collection.py`，按显式给出的章节顺序写入同一套 `books/<书名>/chNN/raw.md` 和 `progress.md`，并对每章做逐字校验。

这只是 OrbitOS 的输入适配层，不改变上面的单文件入库流程，也不代替后续 `chapter-split` 对单章进行阅读单元切分。
