...
- **Node.js 18+**：供部分脚本使用（如 `orbitos-vault.mjs`）
...
```bash
git clone https://github.com/your-org/orbitos-vault-template my-vault
cd my-vault
```
...
| 操作 | 说明 | 可跳过 |
|------|------|--------|
| ✅ 结构验证 | 确认 `.orbit/schema/` 6 个文件完整 | 否 |
...
1. 设置（左下角齿轮）→ 第三方插件
2. 关闭「安全模式」
3. 在已安装插件列表找到 **OrbitOS Dashboard** → 启用
...
| 目录 | 内容 | 触发方式 |
|------|------|---------|
| `工作日志/` | 每日工作记录，git commit 和 Agent 结构化记录都写这里 | `git commit` 自动 / "今天做了……" |
...
|-------|----------------------|------|
| `orbitos-vault` | "知识库"、"OrbitOS"、"vault" | 主入口，路由到具体工作区和子 Skill |
...
3. 询问你今天做了什么，按 `今日重点 / 今日Todo / 重点记录 / 明日计划` 等标准章节整理
...
在 Obsidian 侧边栏点击 📊 图标（或 `Cmd+P` 搜索 OrbitOS）打开控制台。
...
| **TODOS** | 今日工作日志中的待办事项，可在 Dashboard 中直接查看和勾选完成 |
| **TODAY** | 今日工作日志的重点和决策 |
...
- Agent 被要求在停止前记录工作日志
- 格式：`### HH:MM — 标题`，并补 `记录方 / 写入方式 / 会话ID / 来源目录 / 为什么做 / 怎么做的 / 改了什么`
- 写入今日工作日志的 `## 重点记录` 章节
...
编辑 `.orbit/workspace-index.yaml`，修改 `desc` 字段（保持 `dir` 不变）：
...
编辑 `00-系统/Skills/orbitos-vault/SKILL.md`，在 `triggers:` 下添加：
...
编辑 `.orbit/schema/taxonomy.yaml` 添加自定义 topic：
...
在 vault 根目录（含 `.orbit/` 的目录）打开 Agent。或者：

```bash
export ORBIT_VAULT=/path/to/my-vault
```
...
将以下内容复制到你的 vault：
1. `.orbit/` 目录（含所有 schema）
2. `00-系统/Skills/` 目录
3. `CLAUDE.md` 和 `AGENTS.md`
4. `.obsidian/plugins/orbit-dashboard/`
