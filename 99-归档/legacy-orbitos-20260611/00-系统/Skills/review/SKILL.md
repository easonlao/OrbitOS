...
```bash
VAULT=$(cat ~/.orbit/config.yaml 2>/dev/null | grep "^vault:" | awk '{print $2}')
...
for md in vault.rglob("*.md"):
    # 跳过 _legacy、.orbit、WORKSPACE.md 等
    if any(p in str(md) for p in ["_legacy", ".orbit", "WORKSPACE", "AGENTS", "CLAUDE"]):
        continue
