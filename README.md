...
OrbitOS 现在还提供一套动态人物模块：模块本身属于 OrbitOS 的系统能力，但具体的人物档案和本地协作偏好中的派生段落都只落在你当前使用的工作副本里，不会自动变成公共规则。
...
- 如果当前工作副本已经有动态人物档案或本地协作偏好，agent 只会按命中任务按需读取；这些内容属于当前工作副本的本地实例，不等于所有用户通用的系统规则。
